# Context-Based Restrictions (CBR) Configuration for Phase 2
# Implements network-based access control for the VPC infrastructure from Phase 1
...
resource "ibm_cbr_zone" "vpc_internal_zone" {
  ...
  # Dynamically reference the address prefixes from Phase 1
  addresses {
    type  = "ipAddress"
    value = local.address_prefixes["jp-tok-1"] # 10.10.10.0/24
  }
  addresses {
    type  = "ipAddress"
    value = local.address_prefixes["jp-tok-2"] # 10.20.20.0/24
  }
  addresses {
    type  = "ipAddress"
    value = local.address_prefixes["jp-tok-3"] # 10.30.30.0/24
  }
